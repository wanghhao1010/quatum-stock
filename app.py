...
try:
    ...
    st.sidebar.warning("⚠️ 未偵測到 Streamlit Secrets 金鑰，資料庫寫入已暫時關閉。")
    SUPABASE_URL = None
    SUPABASE_KEY = None
    ...
    if df.empty:
        ...
        if len(df) < 40:
            ...
            if is_forced_melt or ensemble_score < 40:
                ...
                ai_situation_analysis = f"當前市場結構處於**嚴重的『高位派發與多頭踐踏期』**。最新小時相較上小時，幾何斜率出現了 {delta_smi_slope_micro:+.4f} 的環比下砸，大戶資金呈無情流出。雖然大盤多數個股上漲，但本股上方賣壓極其沉重，隨時有雪崩風險。"
                ai_institutional_analysis = f"**【大戶黑手戰術拆解】** 跨時對齊確認黑手正上演**『假利多掩護、限價大單出貨劇本』**。大戶利用大盤普漲的樂觀氛圍在早盤拉高，隨後在盤中執行高頻派發。AI 自適應大腦已自動提升防守權重，強制熔斷交易號令以鎖定你的本金成本。"
            elif ensemble_score >= 75:
                sys3_css, sys3_status = "bg-ens-buy", "🔴 SYSTEM 3：強烈買進"
                sys3_pos = "80% ~ 100% 滿倉重擊突破"
                action_signal = "STRONG_BUY"
                sys3_desc = f"<b>統合決定：</b>黃金閃擊突破！熱度、幾何、跨天跨時雙時間軸加速度全面共振（評分達 {ensemble_score}）。"
